...
def detectWear(imgName, numW):
	...
	canny = cv2.Canny(gray, 380, 80, apertureSize = 3)
	...
	for i in range (0,4):
		...
		for file in os.listdir(f[i]):
			if file.endswith(".jpg"):
				wirePhotos[i].append(os.path.join(f[i], file))

		wirePhotos[i] = sorted(wirePhotos[i])
...
#i += detectWear(curName)

#i += detectCorrosion(imgName1)
